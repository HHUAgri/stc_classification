...
class AttentionBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        # value_attentioned: [N, in_channels, T], weight_temp: [N, T, T]
        return value_attentioned, weight_temp


class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding,
                 key_size, num_sub_blocks, temp_attn, nheads, en_res, conv, vhdrop_layer, visual, dropout=0.2):
    ...
    def __init__(self, input_output_size, emb_size, num_channels, num_sub_blocks, temp_attn, nheads, en_res,
        ...
        for i in range(num_levels):
            ...
            layers += [TemporalBlock(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size, padding=(kernel_size-1) * dilation_size,
                                     key_size, num_sub_blocks, temp_attn, nheads, en_res, conv, vhdrop_layer=self.vhdrop_layer, visual=visual, dropout=dropout)]
