...
class SpatialBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, dilation, dropout=0.2):
...
class DenseSpatialBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, dropout=0.2):
        ...
        self.sb1 = SpatialBlock(in_channels[0], n_outputs=out_channels[0], dilation=1)
        self.sb2 = SpatialBlock(in_channels[1], n_outputs=out_channels[1], dilation=2)
        self.sb3 = SpatialBlock(in_channels[2], n_outputs=out_channels[2], dilation=3)
...
class VectorBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, padding=0, dropout=0.2):
...
class DenseSpatialNet(nn.Module):
    def __init__(self, n_inputs=4, n_outputs=256):
        super(DenseSpatialNet, self).__init__()

        self.embedding_layer = SpatialEmbedding(n_inputs, n_outputs)
        self.dense_layer1 = DenseSpatialBlock(n_outputs, n_outputs)
        self.dense_layer2 = DenseSpatialBlock(n_outputs, n_outputs)
        self.attention_layer = ChannelSpatialSELayer(n_outputs)
        self.vector_layer = VectorBlock(n_outputs, n_outputs, padding=1)
